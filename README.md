...
## 🆘 Troubleshooting: Admin Tidak Bisa Login

Gejala: password benar, baris `public.users` sudah ber-role `ADMIN` dan `APPROVED`, tetapi aplikasi tetap kembali ke halaman login atau menampilkan pesan "User ID atau Password yang Anda masukkan salah".

Jalankan diagnosis read-only [`supabase/check_admin_login.sql`](supabase/check_admin_login.sql) di Supabase SQL Editor, lalu cocokkan hasilnya dengan tabel berikut:

| Penyebab | Terlihat dari | Perbaikan |
|---|---|---|
| Kolom `users.role_id`/`section_id` atau foreign key ke `roles`/`sections` belum ada. Permintaan profil `select('*, roles(id, name), sections(id, name)')` ditolak PostgREST dengan `PGRST200`, sehingga dashboard gagal dimuat dan sesi langsung di-signOut. | `kolom_role_section_ada` / `fk_role_section_ada` = `false` | `supabase/fix_admin_login.sql` (Bagian A). Frontend versi terbaru sudah memakai fallback kolom dasar, tetapi skema tetap harus diselaraskan. |
| Email Supabase Auth belum dikonfirmasi sementara project memakai **Confirm email** aktif (`mailer_autoconfirm = false`). Domain `@outing.local` tidak bisa menerima email konfirmasi. | `email_confirmed` = `false` (pesan Auth: *Email not confirmed*) | `supabase/fix_admin_login.sql` (Bagian B). |
| Akun Auth `admin@outing.local` belum dibuat, atau dibuat lewat `INSERT` manual tanpa baris `auth.identities`. | `auth_ada` / `identity_email_ada` = `false` | Buat user di **Authentication → Users → Add user**, lalu jalankan `supabase/fix_admin_login.sql` (Bagian B). |
| Profil `public.users` belum terhubung ke akun Auth (`auth_user_id` NULL) atau masih `PENDING`. | `profil_terhubung_ke_auth` / `ada_profil_admin_approved` = `false` | `supabase/fix_admin_login.sql` (Bagian C). |
| Tabel modul (mis. `donation_*`) belum dibuat. | Hasil bagian 6 pada `check_admin_login.sql` | Jalankan `database_schema.sql` sampai selesai. Modul yang gagal tidak lagi menggagalkan login sejak setiap langkah dashboard diisolasi. |

Catatan tambahan:

- Setelah menjalankan script perbaikan, PostgREST dimuat ulang otomatis (`NOTIFY pgrst, 'reload schema'`). Jika masih memakai cache lama, tunggu beberapa detik atau buka **Project Settings → API → Reload schema cache**.
- Login memakai **User ID** `admin` (aplikasi memetakannya ke email internal `admin@outing.local`). Mengetik alamat email lengkap juga didukung.
- Bila diagnosis menyatakan `identity_email_ada = false` pada project baru, jalankan `supabase/seed.sql` yang membuat baris `auth.identities` untuk seluruh akun demo.
