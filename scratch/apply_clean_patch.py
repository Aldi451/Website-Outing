# Python script to cleanly patch index.html from backup with all JS inside the main script
...
html = html.replace('</style>', css_additions, 1)

# 2. Home Action Toolbar
...
# 5. Add Archive Core Storage Helpers before initSeedDataIfEmpty()
old_init_seed = """// INITIAL SEED DATA
function initSeedDataIfEmpty() {"""

new_init_seed = """// ARCHIVE CORE STORAGE HELPERS
...
html = html.replace(old_init_seed, new_init_seed, 1)

# 6. Update setupManagementMenu() to include Softcopy Report & Arsip tile
...
# 8. Place JS Engine INSIDE the main script, right before `// Start application\ncheckSession();`
old_check_session = """// Start application
checkSession();"""
...
html = html.replace(old_check_session, js_engine_code, 1)

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print("Unified patch applied successfully!")
