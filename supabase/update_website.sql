-- UPDATE LENGKAP WEBSITE OUTING — 2026-09-23
...
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS role VARCHAR(50) DEFAULT 'PARTICIPANT';
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS section VARCHAR(50) DEFAULT 'PUBLIC';
...
CREATE TABLE IF NOT EXISTS public.donation_incident_cases (
    ...
    -- Nama berbeda dari CHECK otomatis kolom approved_amount di atas.
    CONSTRAINT donation_incident_cases_approved_amount_limit_check CHECK (approved_amount IS NULL OR approved_amount <= requested_amount)
);
...
REVOKE ALL ON FUNCTION public.current_app_user_is_approved() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.current_app_user_is_admin() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.current_app_user_is_active_donor() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.current_app_user_has_role(TEXT[]) FROM PUBLIC, anon;
...
BEGIN
    ...
    IF COALESCE(NEW.raw_user_meta_data ->> 'outing_id', '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' THEN
        -- Cache browser mungkin menunjuk outing yang sudah dihapus.
        SELECT id INTO v_outing_id FROM public.outings
        WHERE id = (NEW.raw_user_meta_data ->> 'outing_id')::UUID;
    ELSE
        v_outing_id := NULL;
    END IF;
...
BEGIN
    ...
    IF v_status = 'APPROVED' AND v_user.auth_user_id IS NOT NULL THEN
        UPDATE auth.users
        SET email_confirmed_at = COALESCE(email_confirmed_at, NOW()), updated_at = NOW()
        WHERE id = v_user.auth_user_id
          AND lower(email) = lower(v_user.username) || '@outing.local';
    END IF;
...
REVOKE ALL ON FUNCTION public.admin_set_user_approval(UUID, TEXT, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_set_user_approval(UUID, TEXT, TEXT) TO authenticated;
REVOKE ALL ON FUNCTION public.admin_enqueue_donation_reminders() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_enqueue_donation_reminders() TO authenticated;
REVOKE ALL ON FUNCTION public.admin_prepare_donation_year_closure(UUID, UUID, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_prepare_donation_year_closure(UUID, UUID, TEXT) TO authenticated;
REVOKE ALL ON FUNCTION public.admin_mark_foundation_disbursement_paid(UUID, DATE, TEXT, TEXT, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_mark_foundation_disbursement_paid(UUID, DATE, TEXT, TEXT, TEXT) TO authenticated;
REVOKE ALL ON FUNCTION public.enqueue_due_donation_reminders(TIMESTAMPTZ) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.claim_donation_reminders(INTEGER) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.complete_donation_reminder(UUID, TEXT, TEXT, TEXT) FROM PUBLIC, anon;
...
-- Explicit grants: projects with customized default privileges also work.
GRANT USAGE ON SCHEMA public TO authenticated;
REVOKE ALL ON public.users FROM anon, authenticated;
GRANT SELECT ON public.users, public.roles, public.sections TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.outings, public.outing_users,
    public.rundowns, public.cash_accounts, public.cash_transactions,
    public.purchase_requests, public.tasks, public.consumption_plans,
    public.announcements, public.participants TO authenticated;
...
-- BEGIN GENERATED WEBSITE MODULES (scripts/build_sql_update.py)
-- SOURCE: supabase/participant_management.sql
-- ====================================================================
...
-- SOURCE: supabase/registration_notifications.sql
